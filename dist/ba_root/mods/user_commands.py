    ...
    if len(msg.split()) < 2:
        ...
        if urself:
            ...
            if specific_player_stats and len(specific_player_stats) == 6:
                ...
                bs.broadcastmessage(
                    f"{v2_id} | Rank: {rank} | Score: {score} | Kills: {kills} | Deaths: {deaths} | K/D: {kd:.2f} | Games Played: {games_played}",
                    transient=True,
                    color=(1, 1, 1),
                    clients=None,
                )
...
def pb(msg, client_id):
    if len(msg.split()) < 2:
        bs.broadcastmessage(
            "/pb <id>", transient=True, color=(1, 0, 0), clients=[client_id]
        )
        return None
    else:
        args = msg.split()
...
def help(msg, client_id):
    ...
    try:
        ...
        admin_chunks = [
            admin_commands[i : i + 10] for i in range(0, len(admin_commands), 10)
        ]
        message += (
            "Admin Commands: "
        ...
        user_chunks = [
            user_commands[i : i + 10] for i in range(0, len(user_commands), 10)
        ]
        message += (
            "User Commands: "
